...
def test_coerce_numeric_value_fractions_and_lists():
    from ocr_llm.repairs.common import _coerce_numeric_value
    # String fractions
    assert abs(_coerce_numeric_value("1/3") - 0.3333333333333333) < 1e-9
    assert abs(_coerce_numeric_value(" 2 / 5 ") - 0.4) < 1e-9
    # List / tuple fractions
    assert abs(_coerce_numeric_value([1, 4]) - 0.25) < 1e-9
    assert abs(_coerce_numeric_value((3, 6)) - 0.5) < 1e-9
    # Nested/invalid lists
    assert _coerce_numeric_value([1, "xyz"]) == [1, "xyz"]
    # Single element lists
    assert _coerce_numeric_value([5]) == 5.0
    assert _coerce_numeric_value(["1/4"]) == 0.25
